...
const App: React.FC = () => {
  // Group members by units for a filtered view if needed, but the design implies a grid
  // We will show a "Featured News" style header for the member section
      ...
      <main>
        ...
        <section id="members" className="py-24 px-6 max-w-[1920px] mx-auto border-t border-gray-100">
            ...
            <div className="flex gap-8 mb-12 overflow-x-auto pb-4 border-b border-gray-100">
                {['ALL', 'HIP-HOP', 'VOCAL', 'PERFORMANCE'].map((tab, i) => (
                    <button 
                        key={tab} 
                        className={`font-display text-lg uppercase tracking-wider whitespace-nowrap ${i === 0 ? 'text-svt-pink' : 'text-gray-400 hover:text-black'}`}
                    >
                        {tab}
                    </button>
                ))}
